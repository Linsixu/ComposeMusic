...
fun DefaultTeacherNameScreen(
    ...
    LaunchedEffect(Unit) {
        val savedValue = spStorage.getString(DEFAULT_TEACHER_NAME)
...
// 顶部栏：返回按钮 + 居中标题（同一行）
            ...
            Column(
                ...
                Button(
                    onClick = {
                        when {
                            inputValue.isBlank() -> submitTips = "请输入默认教师名称"
//                            inputValue.toIntOrNull() == null -> submitTips = "请输入有效的数字"
                            else -> {
                                focusManager.clearFocus()
                                SPKeyUtils.currentTeacherName.value = inputValue
                                spStorage.saveString(DEFAULT_TEACHER_NAME, inputValue)
                                submitTips = "保存成功"
                                showToast("保存成功")
